const useDialSipEndpointMethod =
	Number(process.env.USE_DIAL_SIP_ENDPOINT_METHOD) || 0
// const assert = require('assert')
const { registerCall } = require('../../lib/utils')
const getEnvVariables = require('./users')
...
const service = ({ logger, makeService }) => {
	...
	svc.on('session:new', async (session) => {
		...
		if (
			...
			callParams = getEnvVariables(from)

			if (callParams.PSTN_TRUNK_NAME && callParams.RETELL_SIP_CLIENT_USERNAME) {
				/* check if the call is coming from Retell; i.e. using the sip credential we provisioned there */
				const username =
					session.sip.headers['X-Authenticated-User'].split('@')[0]
				if (username === callParams.RETELL_SIP_CLIENT_USERNAME) {
		...
		try {
			...
			if (outboundFromRetell) {
				...
				target = [
					{
						type: 'phone',
						number: to,
						trunk: callParams.PSTN_TRUNK_NAME,
					},
				]
				...
				callParams = getEnvVariables(to)
				target = [
					{
						type: 'phone',
						number: to,
						trunk: callParams.RETELL_TRUNK_NAME,
					},
				]
			}
			...
			session.locals.logger.info(
				{ err },
				`Error to responding to incoming call: ${session.call_sid}`
			)
			session.close()
		}
	})
